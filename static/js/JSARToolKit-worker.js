...
var window = {};
var MSG_TYPE = {INIT:1, DETECT:2, CURSOR: 3};
var CURSOR_STATUS = {HIDDEN: 1, SHOW_LEFT: 2, SHOW_RIGHT: 3};

// var recv_msg = {type:'1',canvas_width: '', canvas_height:'',};
// var recv_msg = {type:'2',pixel: '', look:'',};

// var send_msg = {type:'3',stat: ''};
...
( function ( ) {
    ...
    CvWorker.prototype = {
        constructor: CvWorker,

        init: function (data) {
            this.detector = new FLARMultiIdMarkerDetector(new FLARParam(data.canvas_width,data.canvas_height), 80);
            this.detector.setContinueMode(true);
            this.canvas = {width:data.canvas_width, height:data.canvas_height, changed: false};
            this.raster = new NyARRgbRaster_Canvas2D(this.canvas);
            this.log("finish init");
        },

        detect: function (data) {
            this.raster._rgb_reader._data = data.pixel;
            this.cameraWorldDirection = data.look;
            var result = this.detector.detectMarkerLite(this.raster, 170);
            if (!result) {
                this.send_cursor_msg(CURSOR_STATUS.HIDDEN)
            }
            this.log("finish detect");
        },
        ...
        found: function (x, y) {
            ...
            if (Math.abs(rad) < Math.PI/4 || (2*Math.PI-Math.abs(rad)) < Math.PI/4) {
                ...
                this.send_cursor_msg(CURSOR_STATUS.SHOW_LEFT)
            } else {
                this.send_cursor_msg(CURSOR_STATUS.SHOW_RIGHT)
            }
        },

        send_cursor_msg: function (stat) {
            postMessage({type:MSG_TYPE.CURSOR, stat: stat});
        },
...
var cv_worker = new CvWorker();

onmessage = function( event ){
    switch (event.data.type) {
        case MSG_TYPE.INIT:
            cv_worker.init(event.data);
            break;
        case MSG_TYPE.DETECT:
            cv_worker.detect(event.data);
            break;
    }
};
...
FLARMultiIdMarkerDetectCB = ASKlass('FLARMultiIdMarkerDetectCB',
{
  ...
  ,onSquareDetect : function(i_sender,i_coordx,i_coordy,i_coor_num,i_vertex_index)
  {
    ...
    if (window.DEBUG) {
      ...
      //by sohow
      if (cv_worker instanceof CvWorker) {
          cv_worker.found(cx, cy);
      }
